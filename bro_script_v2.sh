...
do
	...
	(sleep 15 &&  timeout -s SIGINT 2 nc omanyte $PORT_START &) # After 15 sec, send signal to start
	(nc -d -l $PORT_STOP && pkill --signal SIGINT -f "./bro/build/src/bro -C -i $INTERFACE count_packets.bro" &) # Receives signal to stop bro

	if [ $MULTICORE -gt 0 ]; then 
		perf stat -o $PERF_INTER ./bro/build/src/bro -C -i $INTERFACE $BRO_SCRIPTS > $BRO_INTER
	else
		taskset 0x4 perf stat -o $PERF_INTER ./bro/build/src/bro -C -i $INTERFACE $BRO_SCRIPTS > $BRO_INTER
	fi
