...
do
	...
	SPEED=$(awk -v m=$MULTIPLIER 'BEGIN { print (m/1.3) }')
	timeout -s SIGINT $TIMEOUT ./MoonGen/build/MoonGen MoonGen/libmoon/examples/pktgen.lua -r $SPEED $INTERFACE > $INTERMEDIATE
